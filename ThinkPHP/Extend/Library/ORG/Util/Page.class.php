...
class Page {
    ...
    protected $config  =    array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'首页','last'=>'尾页','theme'=>'%upPage% %first% %linkPage% %end% %downPage%');
    // 默认分页变量名
    protected $varPage;
    ...
    public function show() {
        ...
        if($this->nowPage > 3 && $this->totalPages > 3) {
            $theFirst   =   "<a href='".str_replace('__PAGE__',1,$url)."' >1</a>";
            $theFirst .= "<span>…</span>";
        }

        if($this->nowPage  + 3  < $this->totalPages && $this->totalPages > 3) {
            $theEnd = "<span>…</span>";
            $theEnd .=   "<a href='".str_replace('__PAGE__',$theEndRow,$url)."' >".$this->totalPages."</a>";
        }
        ...
        // 1 2 3 4 5
        $linkPage = "";
        $start = $this->nowPage - 1 == 0 ? 1 : $this->nowPage - 1;
        $end = $this->nowPage + 1 > $this->totalPages ? $this->totalPages : $this->nowPage + 1;
        if($start == 1) $end = 3;
        for($i = $start; $i<= $end; $i++) {
             if($i !=$this->nowPage){
